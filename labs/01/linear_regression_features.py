...
def main(args: argparse.Namespace) -> list[float]:
    ...
    rmses = []
    # print("This is ys before the loop:", ys)
    ...
    for order in range(1, args.range + 1):
        ...
        f = np.column_stack([xs ** i for i in range(1, order + 1)])
    # print("This is features after the loop:", features)
        ...
        train_data, test_data, train_target, test_target = sklearn.model_selection.train_test_split(
            f, ys, test_size=args.test_size, random_state=args.seed)
        ...
        rmse = np.sqrt(sklearn.metrics.mean_squared_error(test_target, test_predictions))
