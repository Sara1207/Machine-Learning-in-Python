...
def main(args: argparse.Namespace) -> tuple[tuple[np.ndarray, ...], list[float]]:
    ...
    def forward(inputs):
        ...
        ReLU = np.maximum(inputs @ weights[0] + biases[0], 0)

        # Compute the value of the output layer.
        softmax = ReLU @ weights[1] + biases[1]
        softmax = softmax - np.max(softmax, axis=1, keepdims=True)
    ...
    for epoch in range(args.epochs):
        ...
        for start in range(0, train_data.shape[0], args.batch_size):
            ...
            ReLU, softmax = forward(train_data[batch_indices])

            # Compute the derivative of the loss with respect to the inputs of the softmax on the last layer.
            softmax[np.arange(args.batch_size),
                    train_target[batch_indices]] -= 1
            softmax = softmax / args.batch_size

            # Compute the gradients of the weights and biases for both layers.
            gradient_w1 = ReLU.T @ softmax
            gradient_b1 = np.sum(softmax, axis=0)
            gradient_hidden = softmax @ weights[1].T
            gradient_hidden[ReLU <= 0] = 0
            gradient_w0 = train_data[batch_indices].T @ gradient_hidden
            gradient_b0 = np.sum(gradient_hidden, axis=0)

            # Update the weights and biases using these gradients.
            weights[0] -= args.learning_rate * gradient_w0
            weights[1] -= args.learning_rate * gradient_w1
            biases[0] -= args.learning_rate * gradient_b0
            biases[1] -= args.learning_rate * gradient_b1

        # After the SGD epoch, measure the accuracy for both the train test and the test set.
        train_pred = np.argmax(forward(train_data)[1], axis=1)
        train_accuracy = sklearn.metrics.accuracy_score(
            train_target, train_pred)
        test_pred = np.argmax(forward(test_data)[1], axis=1)
