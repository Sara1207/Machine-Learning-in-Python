...
def main(args: argparse.Namespace) -> tuple[list[float], float, float]:
    ...
    for epoch in range(args.epochs):
        ...
        for batch in range(0, len(train_data), args.batch_size):
            ...
            gradient = np.dot(batch_data.T, batch_data @
                              weights - batch_target) / len(batch_data)

            # Add the regularization
            regularization_term = args.l2 * weights
        ...
        # Calculate current RMSE on train/test to train_rmses/test_rmses.
        prediction_train = train_data @ weights
        prediction_test = test_data @ weights

        # Calculate the RMSE
        train_rmse = sklearn.metrics.mean_squared_error(
            train_target, prediction_train, squared=False)
        test_rmse = sklearn.metrics.mean_squared_error(
            test_target, prediction_test, squared=False)
    ...
    explicit_rmse = sklearn.metrics.mean_squared_error(
        test_target, sklearn.linear_model.LinearRegression().fit(train_data, train_target).predict(test_data), squared=False)
...
if __name__ == "__main__":
    ...
    print("Test RMSE: SGD {:.3f}, explicit {:.1f}".format(
        sgd_rmse, explicit_rmse))
    print("Learned weights:", *("{:.3f}".format(weight)
          for weight in weights[:12]), "...")
